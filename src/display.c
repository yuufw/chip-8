...
void
display_init(Display *disp)
{
	for (uint32_t i = 0; i < (uint32_t)(DISPLAY_WIDTH * DISPLAY_HEIGHT);
	     i++) {
		disp->pixels[i] = 0u;
	}
}
...
uint8_t
display_draw_sprite(Display *disp, Memory *m, uint16_t I, uint8_t x, uint8_t y,
                    uint8_t n)
{
	uint8_t collision = 0;
